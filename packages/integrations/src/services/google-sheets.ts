...
function loadSavedTokens(): { refresh_token?: string; access_token?: string } | null {
  const credentialsPath = join('.marktoflow', 'credentials', 'google-sheets.json');
...
export const GoogleSheetsInitializer: SDKInitializer = {
  async initialize(_module: unknown, config: ToolConfig): Promise<unknown> {
    ...
    const actions = new GoogleSheetsActions(sheets);
    ...
    // Copy all methods from GoogleSheetsActions prototype and bind them to maintain 'this' context
